...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Test for getView()
        val sampleCollection = listOf(10, 20, 30)
        val newView = getView(1, null, sampleCollection, this)
        (newView as? TextView)?.let {
            Log.d("function output", "getView() new view text: ${it.text}")  // expected "20"
        }

        // Test recycling a view:
        val recycledTextView = TextView(this).apply {
            setPadding(5, 10, 10, 0)
            textSize = 22f
            text = "Initial"
        }
        val recycledViewResult = getView(2, recycledTextView, sampleCollection, this)
        (recycledViewResult as? TextView)?.let {
            Log.d("function output", "getView() recycled view text: ${it.text}")  // expected "30"
        }
    }
...
//    }
    ...
    private fun getTestDataArray() = MutableList(10) { Random.nextInt() }.also { it.sort() }
...
//    }
    ...
    private fun averageLessThanMedian(listOfNumbers: List<Double>) =
        listOfNumbers.sorted().let { sortedList ->
            listOfNumbers.average() < if (sortedList.size % 2 == 0)
                (sortedList[sortedList.size / 2] + sortedList[(sortedList.size - 1) / 2]) / 2
            else
                sortedList[sortedList.size / 2]
        }
